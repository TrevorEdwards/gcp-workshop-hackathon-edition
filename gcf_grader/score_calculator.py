...
def run_test_cases(elem, **kwargs):
  ...
  for i, csv_test_case in enumerate(test_cases):
    ...
    r = requests.post(full_url, json={'data':test_case}, timeout=10) # TODO parameterize by test number
...
def run(argv=None):
  ...
  with beam.Pipeline(options=pipeline_options) as p:
    ...
    test_cases = p | 'Read test cases' >> ReadFromText(known_args.test_cases + known_args.test_number)
    run_tests = (
        gcf_urls
        | 'Run Tests' >> (beam.transforms.core.ParDo(beam.transforms.core.DoFn.from_callable(run_test_cases), SideInputParam=beam.pvalue.AsIter(test_cases)))
